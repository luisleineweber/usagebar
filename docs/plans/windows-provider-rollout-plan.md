...
| OpenCode Go | Working on Windows | official usage API with local `auth.json`; SQLite and `ccusage` history | self-testable | Bearer auth, API window mapping, entitlement errors, and local-history paths are covered by focused tests | Real account endpoint behavior still needs live validation | Keep the API response contract and Windows auth paths aligned with OpenCode changes | Direct API quota path plus local history keeps the Windows implementation small |
