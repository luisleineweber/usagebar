...
- The March 16 "missing provider" gap is now partly closed in this fork. UsageBar already surfaces blocked Windows placeholders for `augment`, `kiro`, and `vertex-ai`; the current worktree also contains an `alibaba` placeholder, while `kilo`, `synthetic`, `openrouter`, `kimi-k2`, and `warp` have moved to implemented Windows-experimental token/API-key providers.
...
- The old "missing provider" list is no longer accurate in this fork. `augment`, `kiro`, and `vertex-ai` are still surfaced as blocked Windows placeholders in `plugins/*/plugin.json`, while `kilo`, `synthetic`, `openrouter`, `kimi-k2`, and `warp` are now implemented as Windows-experimental direct token/API providers.
