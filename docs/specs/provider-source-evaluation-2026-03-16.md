...
- The March 16 "missing provider" gap is now partly closed in this fork. UsageBar already surfaces blocked Windows placeholders for `augment`, `kilo`, `kiro`, `synthetic`, and `vertex-ai`; the current worktree also contains an `alibaba` placeholder, while `openrouter`, `kimi-k2`, and `warp` have moved to implemented Windows-experimental token/API-key providers.
...
| Kimi K2 | Upstream: not present. Current UsageBar: `api` via `plugins/kimi-k2/plugin.js`, stored provider secret `apiKey`, and `KIMI_K2_API_KEY` / `KIMI_API_KEY` / `KIMI_KEY`. | `api`: API key from settings/env to credits endpoint. Evidence: `plugins/kimi-k2/plugin.js`, `docs/providers/kimi-k2.md`, `D:/usagebar/CodexBar/docs/kimi-k2.md`. | Strong API-key Windows provider. The remaining gap is live-account validation, not missing integration code. | Keep the direct credits contract under test and collect real Windows API-key validation notes before promotion. |
...
| Warp | Upstream: not present. Current UsageBar: `api` via `plugins/warp/plugin.js`, stored provider secret `token`, and `WARP_API_KEY` / `WARP_TOKEN`. | `api`: API token from settings/env to Warp GraphQL limits. Evidence: `plugins/warp/plugin.js`, `docs/providers/warp.md`, `D:/usagebar/CodexBar/docs/warp.md`. | Strong token-based Windows provider. The remaining gap is live-account validation, not missing integration code. | Keep the direct GraphQL contract under test and collect real Windows token validation notes before promotion. |
...
- The old "missing provider" list is no longer accurate in this fork. `augment`, `kilo`, `kiro`, `synthetic`, and `vertex-ai` are still surfaced as blocked Windows placeholders in `plugins/*/plugin.json`, while `openrouter`, `kimi-k2`, and `warp` are now implemented as Windows-experimental direct token/API providers.
...
- The highest-confidence next implementations remain explicit API or local-file providers: `synthetic`, `vertex-ai`, maybe `kilo`; `openrouter`, `kimi-k2`, and `warp` have already moved into the implemented experimental bucket.
