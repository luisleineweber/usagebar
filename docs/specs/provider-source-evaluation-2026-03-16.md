...
- The March 16 "missing provider" gap is now mostly closed in this fork. UsageBar still contains stale placeholder status trails for `alibaba`, while `kiro`, `augment`, `vertex-ai`, `kilo`, `synthetic`, `openrouter`, `kimi-k2`, and `warp` have moved to implemented Windows-experimental providers.
...
- The old "missing provider" list is no longer accurate in this fork. `kiro`, `augment`, `vertex-ai`, `kilo`, `synthetic`, `openrouter`, `kimi-k2`, and `warp` are now implemented as Windows-experimental providers. `alibaba` also has implementation code in this worktree but still needs its stale placeholder trail and test gap resolved.
