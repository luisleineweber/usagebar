...
- The March 16 "missing provider" gap is now mostly closed in this fork. `alibaba`, `kiro`, `augment`, `vertex-ai`, `kilo`, `synthetic`, `openrouter`, `kimi-k2`, and `warp` have moved to implemented Windows-experimental providers.
...
| --- | --- | --- | --- | --- |
| Alibaba Coding Plan | Upstream: not present. Current UsageBar: `api` via stored provider secret `apiKey` or `ALIBABA_API_KEY`, with `ALIBABA_REGION` endpoint selection and request-count quota bars. Evidence: `plugins/alibaba/plugin.js`, `docs/providers/alibaba.md`, `plugins/alibaba/plugin.json`. | `web` + `cookies` primary, `api` secondary: Model Studio/Bailian console session baseline with region-aware API fallback. Evidence: `D:/usagebar/codexbar/docs/alibaba-coding-plan.md`. | New CodexBar-only provider. The fork now ships the explicit API-key path and avoids browser-cookie import as the Windows v1. | Keep the API-key path on Windows and collect real account validation. Add app-owned WebView2 session only if API-key quota access is insufficient. |
...
| Kiro | Upstream/current UsageBar: `json/file` + `sqlite` + `oauth/api`: reads Kiro desktop auth under `~/.aws/sso/cache/kiro-auth-token.json`, local `state.vscdb`, `q-client.log`, and only falls back to live Kiro/AWS usage APIs when local data is missing or stale. Current UsageBar adds Windows/Linux paths using host-expandable `~/...` syntax, a local CLI session metering fallback from `~/.kiro/sessions/cli/*.json`, and marks the provider experimental. Evidence: `plugins/kiro/plugin.js`, `docs/providers/kiro.md`, `plugins/kiro/plugin.json`. | `cli`: runs `kiro-cli chat --no-interactive "/usage"` and parses output. Evidence: `D:/usagebar/codexbar/docs/providers.md`. | Two viable source models. The local desktop-data path is richer because it can expose credits, bonus credits, plan, and overage state. The file-based CLI session fallback is simpler and useful on CLI-only Windows installs, but it is lower fidelity than desktop state. | Keep local desktop auth/cache/log as primary on Windows. Use local CLI session metering only when desktop auth/cache data is absent. |
...
- The old "missing provider" list is no longer accurate in this fork. `alibaba`, `kiro`, `augment`, `vertex-ai`, `kilo`, `synthetic`, `openrouter`, `kimi-k2`, and `warp` are now implemented as Windows-experimental providers.
