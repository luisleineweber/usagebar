...
function isBoundaryParser(node: ParameterOwner): boolean {
  const returnType =
    "returnType" in node && node.returnType !== null && node.returnType !== undefined
      ? node.returnType.typeAnnotation
      : null
  if (returnType?.type === "TSTypePredicate") return true

  const name = functionName(node)
  return name !== null && /^(?:getErrorMessage|is|normalize|parse|sanitize|stringify)/u.test(name)
}
