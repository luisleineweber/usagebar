...
vi.mock("@/lib/report-pricing", async () => {
  const actual =
    await vi.importActual<typeof import("@/lib/report-pricing")>("@/lib/report-pricing")
  return {
    ...actual,
...
import { App } from "@/App"
import { SETTINGS_AUTO_CLOSE_DELAY_MS, SettingsWindowApp } from "@/settings-window-app"
...
describe("App", () => {
  ...
  it("refreshes all enabled providers when clicking next update label", async () => {
    ...
    await waitFor(() => expect(state.startBatchMock.mock.calls.length).toBe(initialCalls + 1))
  ...
  it("queues a follow-up tray update when stable icon update is in-flight", async () => {
    ...
    try {
      ...
      state.traySetIconMock.mockReturnValueOnce(firstSetIcon).mockResolvedValue(undefined)
